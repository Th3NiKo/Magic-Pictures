...
public class GameMechanicMedium : MonoBehaviour {
	...
	GameObject[,] mainTile = new GameObject[10,10];
	...
	void FixedUpdate()
	{
		if (timer >= 0.5f) {
			...
			if (isWinner == 0) {
				...
				if (isCreated == false) {
					Instantiate (WinnerBlock, new Vector3 (winnerX, 0.2f, 60f), Quaternion.identity);
					isCreated = true;
				}
